...
describe("getLaunchCommand", () => {
  ...
  it("uses codex and reviewer prompt by default", () => {
  ...
  it("includes model and prompt when provided", () => {
    const cmd = agent.getLaunchCommand(
      makeLaunchConfig({ model: "qwen3-coder-next-80b", prompt: "Review MR 42" }),
    );
    expect(cmd).toContain("--model 'qwen3-coder-next-80b'");
    expect(cmd).toContain("-- 'Review MR 42'");
  });
});
...
describe("getEnvironment", () => {
  ...
  it("sets AO_SESSION_ID and AO_AGENT_TYPE", () => {
    ...
    expect(env["AO_AGENT_TYPE"]).toBe("reviewer");
  });
});
