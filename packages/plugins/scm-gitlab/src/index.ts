  ...
  return {
    ...
    async detectPR(session: Session, project: ProjectConfig): Promise<PRInfo | null> {
      ...
      try {
        ...
        try {
          ...
          throw new Error(`Failed to parse GitLab MRs: ${err}`);
        }
    ...
    async getPRState(pr: PRInfo): Promise<PRState> {
      ...
      try {
        ...
        throw new Error(`Failed to parse GitLab MR state: ${err}`);
      }
    ...
    async getPRSummary(pr: PRInfo) {
      ...
      try {
        ...
        throw new Error(`Failed to parse GitLab MR summary: ${err}`);
      }
    ...
    async getCIChecks(pr: PRInfo): Promise<CICheck[]> {
      try {
        ...
        try {
          ...
          throw new Error(`Failed to parse GitLab CI jobs: ${err}`);
        }
    ...
    async getReviews(pr: PRInfo): Promise<Review[]> {
      ...
      try {
        ...
        throw new Error(`Failed to parse GitLab MR approvals: ${err}`);
      }
    ...
    async getPendingComments(pr: PRInfo): Promise<ReviewComment[]> {
      ...
      return raw
        ...
        .map((block, index) => {
          ...
          const author = authorMatch ? authorMatch[1] : "reviewer";
        ...
        .filter((c) => !BOT_AUTHORS.has(c.author));
    },
