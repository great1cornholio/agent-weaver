...
function toIssue(data: GitLabIssue, project: ProjectConfig): Issue {
  const normalizedIssueUrl = `${resolveGitLabBaseUrl(project)}/${project.repo}/-/issues/${data.iid}`;
  return {
    id: String(data.iid),
    title: data.title,
    description: data.description ?? "",
    url: normalizedIssueUrl,
  ...
  return {
    ...
    async getIssue(identifier: string, project: ProjectConfig): Promise<Issue> {
      ...
      try {
        ...
        throw new Error(`Failed to parse GitLab issue data: ${err}`);
      }
      return toIssue(data, project);
    },
    ...
    async listIssues(filters: IssueFilters, project: ProjectConfig): Promise<Issue[]> {
      ...
      try {
        ...
        throw new Error(`Failed to parse GitLab issues data: ${err}`);
      }
