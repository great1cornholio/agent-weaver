...
async function hasGitLabAuth(): Promise<boolean> {
  const tokenNames = ["GLAB_TOKEN", "GITLAB_TOKEN"];
  const tokenConfigured = tokenNames.some((name) => {
  ...
  try {
    await execFileAsync("glab", ["auth", "status"], { timeout: 5_000 });
    return true;
  } catch {
    return false;
  }
...
export async function runSpawnPreflight(
  ...
  if (options.issueProvided && project.tracker?.plugin === "gitlab") {
    const authConfigured = await hasGitLabAuth();
    if (!authConfigured) {
      messages.push({
        severity: "error",
